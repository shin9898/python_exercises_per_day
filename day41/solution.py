...
class EventManager:
    ...
    def find_event_from_db(self, event_id: str) -> Event | None:
        if not self.conn or not self.cursor:
            print("エラー: データベースに接続されていません。")
            return None
        try:
            sql = f'SELECT event_id, title, date, capacity FROM events WHERE event_id = ?'
            self.cursor.execute(sql, (event_id,))
            row = self.cursor.fetchone()
            if row: # レコードが見つかった場合
                event_id, title, date, capacity = row
                return Event(event_id, title, date, capacity)
    ...
    def register_customer_to_event(self, customer: Customer, event_id: str):
        ...
        event = self.find_event_from_db(event_id)
...
if __name__ == '__main__':
    ...
    # Event検索
    print(event_manager.find_event_from_db("EVT001"))
